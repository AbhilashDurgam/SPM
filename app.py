...
Session(app)
mydb=mysql.connector.connect(host='localhost',user='root',password='Abhi@mysql1289',db='spm')
...
def updatenotes(nid):
    if not session.get('email'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select title,note_content from notes where n_id=%s',[nid])
        note_data=cursor.fetchone()
        if request.method=='POST':
            title=request.form['title']
            content=request.form['content']
            cursor=mydb.cursor(buffered=True) 
            cursor.execute('update notes set title=%s,note_content=%s where n_id=%s',[title,content,nid])
            mydb.commit()#dml command follows tcl commands ,, some cases both are dml commands
            cursor.close()
            flash(f'notes{title} update successfully')
...
def deletenotes(nid):
    if not session.get('email'):
        ...
        mydb.commit()
        cursor.close()
        flash(f'notes{nid} delete successfully')
        return redirect(url_for('panel')) 
...
def fileupload():
    if not session.get('email'):
        ...
        if request.method=='POST':
            ...
            file_data=file.read()
            cursor=mydb.cursor(buffered=True)
            cursor.execute('insert into file_data(file_name,file_data,added_by) values(%s,%s,%s)',[file_name,file_data,added_by])
            mydb.commit()
            cursor.close()
...
def viewall_files():
    if not session.get('email'):
        ...
        added_by=session.get('email')
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select f_id,file_name,created_at from file_data where added_by=%s',[added_by])
        data=cursor.fetchall() 
